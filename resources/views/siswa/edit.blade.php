        ...
        <center>
            ...
            <form action="{{ route('siswa.update', $siswa->nisn) }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <input type="number" class="form-control" id="no_telp" name="no_telp" value="{{ $siswa->no_telp }}" required>
                </div>
                <div class="mb-3">
                    <label for="id_spp" class="form-label">Spp</label>
                    <select class="form-control" id="id_spp" name="id_spp" required>
                        @foreach ($spp as $sp)
                            <option value="{{ $sp->id }}" {{ $siswa->id_spp == $sp->id ? 'selected' : '' }}>{{ $sp->tahun }} - {{ $sp->nominal }}</option>
                        @endforeach
                    </select>
                </div>
